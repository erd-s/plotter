...
def create_circle(
    ...
    for i in range(steps + 1):
        ...
        path_points.append([x + center_origin_x, y + center_origin_y])
    ...
    for i in range(steps + 1):
        ...
        y = center_origin_y - math.sqrt(abs((radius * radius) - (x_length * x_length)))
        x_list.insert(0, x)
        y_list.insert(0, y)
    ...
    for i in range(steps + 1):
        x = (radius / steps) * i
        y = center_origin_y - math.sqrt(abs((radius * radius) - (x * x)))
        path_points.append([center_origin_x - x, y])
...
def create_circle_v2(
    ...
    path_points = circle_path(
        origin_x=center_origin_x, origin_y=center_origin_y, radius=radius
    )
    plotter.draw_path(path_points)
